...
- Структурированный журнал ошибок с фильтрацией по severity (администратор)
...
│  secureStorage  documentTemplates  loggerService          │
├──────────────────────────────────────────────────────────┤
│              Database Layer (expo-sqlite)                 │
│     schema.js → database.js → seed.js                    │
│     SQLite WAL mode, 41 таблица, 47 индексов              │
├──────────────────────────────────────────────────────────┤
│              i18n (i18next)                               │
...
└── src/
    ...
    │   ├── schema.js               # DDL: 41 таблица + 47 индексов (version 4)
    │   ├── database.js             # 80+ функций работы с данными
    ...
    │   └── loggerService.js        # Централизованное логирование ошибок (severity-based)
    │
    ├── navigation/
    ...
    │   │   ├── AuditLogScreen.js
    │   │   ├── ErrorLogScreen.js         # Журнал ошибок (фильтрация, поиск, детали)
...
| Функциональный блок | Доступные операции |
|---|---|
| Дашборд | Устройства, конфликты синхронизации, журнал аудита, счётчик ошибок |
...
| Журнал ошибок | Структурированный лог ошибок: severity (debug/info/warning/error/critical), источник, стек вызовов, экран, пользователь |
...
#### ErrorLogScreen (Журнал ошибок — администратор)
- Статистика ошибок: всего / сегодня / error+critical
- Фильтрация по severity: Critical, Error, Warning, Info
- Полнотекстовый поиск по сообщению, источнику, контексту
- Цветовая индикация severity (полоска + badge)
- Детальный просмотр: source, message, context (JSON), stack trace, экран, пользователь, timestamp
- Очистка старых записей (>30 дней)
- Доступ: AdminHomeScreen (карточка «Ошибки»), SystemSettings → «Журнал ошибок»

#### loggerService (Сервис логирования)
- Централизованная запись ошибок в таблицу `error_log`
- 5 уровней severity: `debug`, `info`, `warning`, `error`, `critical`
- Автоматический захват `user_id` и текущего экрана через `setLoggerUser()` / `setLoggerScreen()`
- Метод `captureException(source, error)` — логирование Error-объектов со stack trace
- Fallback на `console.error` если запись в БД не удалась (предотвращение бесконечного цикла)
...
| Журнал ошибок | 10 | Ошибки: БД, синхронизация, ценообразование, GPS, PDF-генерация |
...
- **Охват:** ~450 ключей перевода (все экраны, навигация, статусы, ошибки, журнал ошибок)
...
### Безопасная загрузка нативных модулей

Перед подключением `react-native-yamap` выполняется проверка `UIManager.getViewManagerConfig('YamapView')`. Если нативный компонент не зарегистрирован (Expo Go, dev client без нативной сборки), модуль не загружается и приложение автоматически использует OpenStreetMap через `react-native-maps`. Это предотвращает краш `Invariant Violation: View config not found for component YamapView`.
